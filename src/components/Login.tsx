    ...
    if (isLoginMode) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setErrors({
            password: data.error || 'Email or password is incorrect',
          });
        }
      } catch (error) {
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Phone Number field - Only show in signup mode */}
            {!isLoginMode && (
              <motion.div
                variants={itemVariants}
                whileFocus={{ scale: 1.02 }}
            ...
            {!isLoginMode && (
              ...
              >
                <Input
                  ...
                  className={`bg-white border-border text-foreground placeholder:text-muted-foreground rounded-lg h-11 sm:h-12 pr-12 transition-all duration-200 focus:border-primary focus:ring-1 focus:ring-primary ${errors.confirmPassword ? 'border-red-500' : ''}`}
                />
                <button
                  type="button"
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
            ...
            >
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg h-11 sm:h-12 text-sm sm:text-base font-medium transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? (
